...
with st.container(border=True):
    ...
    with col_hotspot:
      st.metric("Hotspot Sumsel", value=fu.format_big_number(sumselhs), delta=f'{fu.format_big_number(hs_diff)}%', delta_color="off")


    with col_temp:
      st.metric("Avg Temperatur Sumsel", value=fu.format_big_number(t_avg_now) + " C", delta=f'{t_diff:.2f}%',delta_color="off")

    with  col_presip:
      st.metric("Avg Presipitasi Sumsel ", value=fu.format_big_number(rr_avg_now) +" mm", delta=f'{rr_diff:.2f}%', delta_color="off")
...
with (main_cl):
    with st.container(border=True):
        st.markdown("**Latar Belakang:**<br>"
                      ...
                      "terutama pada kelompok rentan seperti anak-anak dan ibu hamil] yang dapat mengancam :blue[Generasi Masa Depan]", unsafe_allow_html=True)
        ...
        with st.expander("Analisis ISPU"):
             ...
             st.markdown("Dari diagram di atas dapat kita lihat di Kota Palembang pada Bulan Oktober 2023, mayoritas status pencemaran udara berada di tingkat :blue[Tidak Sehat], "
        ...
        with st.expander("Analisis Korrelasi"):
            st.markdown("Untuk Korrelasi ISPU PM 2.5, terdapat :blue[korelasi positif] antara Jumlah Hotspot harian dengan nilai ISPU PM 2.5 harian, "
                     ...
                     "serta dilengkapi dengan data Sosial Ekonomi masyarakat sekitar, untuk penelitian lebih lanjut.", unsafe_allow_html=True)

with st.container(border=True):
    st.subheader("Insight")
    with st.expander("Partikel Kecil Mengancam Generasi Masa Depan"):
       st.markdown("Particulate Matter (PM2.5) adalah partikel udara yang berukuran lebih kecil dari atau sama dengan 2.5 µm (mikrometer). "
             ...
             "Sehingga paparan pencemaran udara pada anak-anak jauh menurun, sehingga meningkatkan kesehatan dan kecerdasan dari :blue[Generasi Masa Depan Indonesia.] ", unsafe_allow_html=True)
